...
$canViewAcademic = hasPermission('view_academic');
$canViewMarketing = hasPermission('view_marketing');
$canViewFinance = hasPermission('view_finance');
// ------------------------------------
...
// --- CRITICAL REDIRECT CHECK: Prevents viewing content without permission ---
// If the current tab is an Academic tab AND the user doesn't have the permission, redirect to a safe tab (AI)
if (in_array($activeTab, $academicTabs) && $activeTab !== 'risk') {
    if (!$canViewAcademic) {
        // Redirect to a page they can see (e.g., Talk with AI)
        header('Location: index.php?active_tab=ai'); 
        exit;
    }
}
// --------------------------------------------------------------------------
...
<div class="dashboard-layout">
    <nav class="sidebar">
        ...
        <?php if (hasPermission('view_academic')): // CRITICAL FIX: Only display the entire master tab if permission is granted ?>
        <details class="master-tab" <?php echo (in_array($activeTab, $academicTabs)) ? 'open' : ''; ?>>
            <summary class="nav-btn master-summary">
                <span>🎓</span> Academic
            </summary>
            <!-- Sub-Tabs (ACADEMIC ACCESS IS GOVERNED BY view_academic PERMISSION) -->
            ...
            <?php if ($isAdmin): // High Risk is Admin only ?>
        ...
        <?php if (hasPermission('view_marketing')): // Check GRANULAR permission ?>
        ...
        <?php if (hasPermission('view_finance')): // Check GRANULAR permission ?>
        ...
        <?php if ($isAdmin): // Maintenance is a core Admin function, keep as Admin only ?>
    ...
    <main class="main-content">
        ...
        <?php 
        // NOTE: The filter bar is only useful for the server-rendered Student List ('list').
        // It must be hidden for all other tabs, including the new API-driven CRUD pages.
        if($activeTab == 'list'): 
        ?>
        ...
        <?php 
            switch($activeTab) {
                ...
                case 'academic_crud':
                    if(file_exists('tab_academic_crud.php')) include 'tab_academic_crud.php';
                    else echo "<div class='card'><h2>Academic Data CRUD</h2><p>File tab_academic_crud.php not found. Please check file name and location.</p></div>";
                    break;
