 ...
 *   CHANGES: Added Session Start, Authentication Check, and hasPermission function.
 * 
 * USED BY: 
...
// --- NEW PERMISSION HELPER FUNCTION (CRITICAL FIX) ---
function hasPermission(string $permission) {
    // 1. Admins always have all permissions
    if (($_SESSION['role'] ?? 'guest') === 'admin') {
        return true;
    }
    // 2. Check if the permission is explicitly set in the session's permission array
    // This array is set in auth.php after a successful login
